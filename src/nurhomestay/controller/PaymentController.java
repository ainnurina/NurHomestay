...
import nurhomestay.dao.CashDAO;
		...
		String action = request.getParameter("action");
		 ...
		 if (action.equalsIgnoreCase("makePayment")) {
			...
			if (method.equalsIgnoreCase("cash")) {
				daoReservation.addReservation(id, home_id, checkin, checkout);
				Reservation res = new Reservation();

				res = daoReservation.getReservationByCustId(id, home_id, checkin, checkout); //
				int reserveid = res.getReserveid();
				System.out.println("reserve id:"+reserveid);
				...
				daoCash.addCash(paymentid);
				
				Cash seecash = new Cash();
				seecash = daoCash.getCashByPaymentId(paymentid);
				...
				if(!response.isCommitted()) {
					forward = "SuccessReservation.jsp";
					request.setAttribute("paid", paid);
					request.setAttribute("catcash", seecash);
		            DecimalFormat priceFormatter = new DecimalFormat("0.00");
		        	request.setAttribute("priceFormatter", priceFormatter); 
				}
